...
- Modeling of _TRPM4_ with the relevant gene sets by GAMs with PRSS and REML, via optimizing parameters including $k$, $\lambda$, and $\gamma$ (the $\gamma$ parameter is manually set and it affects $\lambda$, validated qualitatively via visualizations and quantitatively by 10-fold cross-validation).
