...
- Modeling of _TRPM4_ with the relevant gene sets by GAMs with PRSS and REML, via optimizing parameters including $k$, $\lambda$, and $\gamma$ (manually set and it affects $\lambda$; the $\gamma$ parameter is validated qualitatively through visualizations and quantitatively by 10-fold cross-validation).
